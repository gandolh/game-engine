...
- [x] Differentiate farmer WORK sprites per personality (currently pixel-identical) + idle silhouettes/hats — *added a per-personality HAT SILHOUETTE (flat/spiked/wide-brim/feather-cap) stamped onto every farmer frame via PERSONALITY_HATS in atlas-builder templates; atlas regenerated; work frames now pairwise non-identical (test) + visually confirmed distinct. (2026-06-09)*
...
- [x] Add discoverable focus-on-farmer affordances (hint, "Following: X", bold "Why:", stronger highlight, rename Reset→Unfollow) — *observer/panel.ts: dynamic "Unfollow {name}"/"Reset view" button (badge+rename in one), persistent "click a farmer to follow" hint, bold Why: header, 2px gold outline + tint highlight. Verified live. (2026-06-09)*
