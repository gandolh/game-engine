/**
 * Serializers for the Hollow research CLI's three output files (chunk
 * hollow-07) — `metrics.csv`/`.json`, `events.jsonl`, `lineage.json`. Kept
 * as PURE functions over plain data (no sim access) so they're
 * unit-testable in isolation (see `export.test.ts`) — mirrors
 * `tools/run-sim/src/format.ts`'s `toCsv`/`csvCell` pattern.
 *
 * Fixed key order + fixed decimal precision are both load-bearing:
 * `run-core.ts`'s `MAX_YEARS`/`SEED` re-runs must diff byte-identical (see
 * CLAUDE.md's determinism note), so every column here is written in one
 * unchanging order and every float is rounded to `DECIMALS` places rather
 * than left at floating-point's full (and run-to-run-noise-prone at the
 * last ULP) precision.
 */
