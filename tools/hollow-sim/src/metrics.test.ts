...
describe("gini", () => {
  ...
  it("approaches 1 as n grows for a single holder of all the wealth", () => {
    ...
    values.push(100);
    // Exactly (n-1)/n for a one-holder distribution at any finite n.
...
describe("meanGenes", () => {
  it("computes one mean per BEHAVIOR_GENES entry, in order", () => {
    ...
    // A gene no agent has data for still gets a (0, empty-mean) entry, not `undefined`.
...
describe("sumSocialCounts", () => {
  it("sums the given verb keys and defaults missing ones to 0", () => {
    ...
    expect(sumSocialCounts(counts, COOP_VERBS)).toBe(5); // gift+share+help+teach(0)+trade(0)
