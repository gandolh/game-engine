/**
 * Pure metric computations for the Hollow research CLI (chunk hollow-07) —
 * Gini, mean-trust, mean-gene, and community aggregates, kept as PURE
 * functions over plain data so they're unit-testable without booting a sim
 * (see `metrics.test.ts`). `readLivingAgents` is the one function that
 * actually touches `sim.world` — a READ-ONLY query (never mutates a
 * component, never advances any `Rng`), mirroring `tools/run-sim/src/
 * run-core.ts`'s `summarize()` off-sim-path observer pattern.
 */
import { BEHAVIOR_GENES } from "@hollow/sim-core/components";
import type { BootedHollowSim, HollowSnapshot } from "@hollow/sim-core/sim-bootstrap";
 ...
 *  (for a per-window delta, see `run-core.ts`). */
