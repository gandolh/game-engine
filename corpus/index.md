...
#### Game — 41–46 (all Done as of 2026-06-06)

Gameplay / content / world-depth wave (queued 2026-06-05, all shipped by 2026-06-06; **bold** scope — these re-baselined the determinism outcome by design, same-seed reproducibility verified MATCH ×3 after each; 41 was the spine others built on):
...
- [briefs/game/done/42-livestock-and-orchards.md](briefs/game/done/42-livestock-and-orchards.md) — coops/barns (daily products, care→quality) + perennial orchards. **Done — fires live**: the patient-capital play lets Cora overtake the aggressive leader (first lead crossing; partially closes the leader-runaway gap).
- [briefs/game/done/43-greenhouse-and-farm-skill-progression.md](briefs/game/done/43-greenhouse-and-farm-skill-progression.md) — year-round greenhouse (season-immune plots) + per-farm skills (farming/foraging/fishing/mining). **Done — fires live** (Cora builds it d33, out-of-season corn grows). Balance note: greenhouse doesn't amortize by d100 (Cora drops to #2; runaway returns); skills lopsided to farming.
- [briefs/game/done/44-living-world-working-npcs-and-tavern.md](briefs/game/done/44-living-world-working-npcs-and-tavern.md) — make the cosmetic carpenter/blacksmith fulfill real orders; add a tavern social hub; repurpose the dead notice board. **Done — fires live.** (Surfaced + fixed the latent mining-`Math.random()` determinism bug — see log.)
- [briefs/game/done/45-seasonal-visual-identity-and-festivals.md](briefs/game/done/45-seasonal-visual-identity-and-festivals.md) — season-variant tiles + rain/snow particles (the art ask) + scheduled festival days with contests/special markets. **Done — fires live**: 4 mid-season festivals, a deterministic harvest contest narrated to the feed, agents plan via beliefs.
- [briefs/game/done/46-harbor-shipping-and-contracts.md](briefs/game/done/46-harbor-shipping-and-contracts.md) — a harbor island + time-boxed seeded contracts + reputation: the demand-driven playstyle axis (and a soft fix for the fixed-price-shop flatness). **Done — fires live**: contracts post/commit/deliver/miss; the hoarder fulfills them live (broad four-way uptake is a future balance lever). Part C (bulk export) deferred. **The last brief — game briefs 01–46 are all done.**
