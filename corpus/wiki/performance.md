...
- **Client today** ([sim-client/client.ts](../../games/farm/client/src/worker/sim-client/client.ts)): renders **one tick (~50ms) behind** latest and lerps prev→current with `smoothstep(alpha)` — this is textbook snapshot interpolation and is *correct*, but a **1-tick buffer tolerates zero dropped snapshots** before it underruns.
...
`getInterpolatedSprites()` runs every *frame* (~60fps, not per tick): `new Map()` of prev-sprites, `.map()` a fresh array, spread `{...s}` per sprite — [sim-client/client.ts](../../games/farm/client/src/worker/sim-client/client.ts), [main/render-loop.ts](../../games/farm/client/src/main/render-loop.ts). Classic GC-churn.
