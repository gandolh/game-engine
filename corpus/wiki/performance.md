...
- [x] **Enable `permessage-deflate`** in the `WebSocketServer` options. ✅ **Done 2026-06-10** ([server/src/index.ts](../../packages/server/src/index.ts), `perMessageDeflate: { threshold: 1024 }` — `threshold` skips tiny control frames). ~70–80% size reduction on repetitive JSON at near-zero CPU for 36KB frames. Typecheck clean; transport-only. **Still TODO: verify the Caddy WS reverse-proxy passes compression through in prod** (see [decisions.md](decisions.md) deploy section) or the win evaporates.
- [ ] **Bump render delay 1 tick → 2 ticks (~100ms).** Valve's proven `cl_interp` default; absorbs one dropped snapshot + a ~30ms TCP retransmit instead of freezing. Input latency is irrelevant for a spectator game, so this is free UX insurance. (Lives in `client.ts`'s `renderDelayMs` getter.) *Deliberately left open — it trades 50ms display latency for jitter tolerance, reversing a documented choice in that getter; decide intentionally.*
...
- [ ] **`visibilitychange` handling (correctness).** rAF pauses when the tab is hidden; on return, the wall-clock `alpha` math can spike / the snapshot buffer can avalanche. Pause + resync on `document.hidden` so returning to the tab doesn't stutter. *(Verify whether the client already does this before building.)*
...
5. **T1.3 WebSocket transport quick-wins** (queued 2026-06-10) — `permessage-deflate` + render-delay 1→2 ticks + `setNoDelay` + `visibilitychange`. An afternoon; render/transport-only. Do these together as one commit.
6. **Tier 3 game-feel** (queued 2026-06-10) — pixel-snap + camera smoothing + number popups change *perceived* quality most. Game-feel briefs, not perf.
