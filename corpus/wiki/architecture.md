...
- **Waterfall** ([render-loop.ts](../../packages/farm-valley/src/main/render-loop.ts)): the animated `WATERFALL_FRAMES` overlay at `WATERFALL_TILE` now also emits throttled, on-screen-gated **mist/spray** particles at its foot (upward droplets that arc back + a faint rising mist). Render-only. **Open:** a structural redesign (a *tall* cascade emerging from a rock wall with a rocks backdrop) needs **new atlas frames** — the existing `structure/waterfall` + `waterfall-a/b/c` bake a foam pool into every tile's bottom rows, so they can't be stacked vertically into a continuous fall; that work also wants live visual iteration.
