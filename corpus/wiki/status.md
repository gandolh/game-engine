...
**Where things stand:** engine briefs 01–09 and game briefs 01–80 + **82** are **Done or Superseded**. **[83-visual-depth-polish](../briefs/game/todo/83-visual-depth-polish.md)** has a **first pass on all 4 items shipped** (granular water, sandy-shore descent, bridge guard-rope, cottage eave/form-shadow) — kept in `todo/` pending the user's visual sign-off; item 3's *true* side face + `home`/`forge-house` are deferred to a visual-iteration round (see log.md). The `webgpu-migration` branch was fast-forward-merged into `main` (2026-06-12); the static-layer bake serves both backends, so items 2+4 landed via the Canvas2D `decorate` pass for both (the item-4 shader stays a backlog idea). Tests + typecheck green. **One open thread**, diagnosed, unfixed:
