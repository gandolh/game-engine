...
**Where things stand:** engine briefs 01–09 and game briefs 01–83 + **82** are **Done or Superseded**. **todo/:** [85-animation-engine](../briefs/game/todo/85-animation-engine.md) (reintroduce the `AnimationClip`/`Animator` primitive **with consumers** + give working farmers/Pip an action swing; render-only — see [wiki/animation.md](animation.md)). **Improvement backlog filed 2026-06-12** (ten briefs, see [index.md](../index.md) todo sections): engine **10** WASM allocator fault · **11** WGSL validation guard · **12–16** shader-ideas promotions (day/night wash → living water → weather parity → cloud shadows → wind sway; 12 unblocks 13/15); game **86** juice + number popups (Tier-3 backlog) · **87** home/forge-house restyle leftover · **88** real-VPS deploy verification (blocked on user) · **89** detailed 24×24 characters + held hotbar tool (design agreed, awaiting build sign-off; render-only — see [wiki/animation.md](animation.md)). [83-visual-depth-polish](../briefs/game/done/83-visual-depth-polish.md) is **Done** (user-confirmed visuals, 2026-06-12): granular near-shore water, sandy-shore descent, sagging bridge guard-rope with organic sway, procedural WebGPU water (replaced the tiled-dot "blue rectangles"), and a Stardew-style restyle of all 5 cottages. The game is now **WebGPU-only** (`farm-valley` forces the backend; Canvas2dRenderer kept in `@engine/core` for tests). Minor un-done polish: `home`/`forge-house` (different structures) keep their old look. **Gotcha:** WGSL isn't validated by tsc/vitest — a reserved-keyword identifier black-screened the game; a `wgsl-lint.test.ts` guard now scans for that (log.md). Tests + typecheck green (one atlas cache test times out on constrained hardware while the dev server runs — environmental). **One open thread**, diagnosed, unfixed: The game is now **WebGPU-only** (`farm-valley` forces the WebGPU backend; Canvas2dRenderer kept in `@engine/core` for tests). The `webgpu-migration` branch was fast-forward-merged into `main` (2026-06-12). **Gotcha:** WGSL isn't validated by tsc/vitest — a reserved-keyword identifier black-screened the game; a `wgsl-lint.test.ts` guard now scans for that (see log.md). Tests + typecheck green (one atlas cache test times out on constrained hardware while the dev server runs — environmental). **No open threads.**
