...
**Where things stand:** engine briefs 01–09 and game briefs 01–80 + **82** are **Done or Superseded**. **[83-visual-depth-polish](../briefs/game/todo/83-visual-depth-polish.md)** shipped + iterating: granular near-shore water, sandy-shore descent (both user-confirmed good), sagging bridge guard-rope with organic sway, procedural WebGPU water (replaced the tiled-dot "blue rectangles"), and a Stardew-style restyle of all 5 cottages. Kept in `todo/` — the latest (post-black-screen-fix water + all-5 cottage restyle) awaits the user's final visual confirmation; `home`/`forge-house` (different structures) not restyled. The game is now **WebGPU-only** (`farm-valley` forces the WebGPU backend; Canvas2dRenderer kept in `@engine/core` for tests). The `webgpu-migration` branch was fast-forward-merged into `main` (2026-06-12). **Gotcha:** WGSL isn't validated by tsc/vitest — a reserved-keyword identifier black-screened the game; a `wgsl-lint.test.ts` guard now scans for that (see log.md). Tests + typecheck green (one atlas cache test times out on constrained hardware while the dev server runs — environmental). **One open thread**, diagnosed, unfixed:
