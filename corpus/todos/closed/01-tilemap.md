...
> ⛔ **SUPERSEDED — never built as specified.** This spec targets **raw WebGPU**; the renderer went
> to Canvas2D instead, and then to **WebGL2 only** on 2026-08-18 (both Canvas2D and WebGPU are now
> deleted — see [wiki/decisions.md](../../wiki/decisions.md) → *Renderer*). The chunked-tilemap idea
> it describes was not carried forward either: the static terrain is baked once and drawn as a single
> clipped texture ([`webgl2/static-layer-pass.ts`](../../../engine/core/src/render/webgl2/static-layer-pass.ts)).
>
> *This note was added 2026-09-19 when the `briefs/` tree was folded into `todos/closed/`. It was the
> one superseded spec of twelve whose status lived only in its directory name (`superseded/`), and the
> reason was recorded only in a `status.md` table that audit-58 removed. Recovered from git so the
> file carries its own state, which is the current convention.*
