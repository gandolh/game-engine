...
> **Update 2026-06-30 — framework SHIPPED + ALL 6 consumer todos DONE.** ([brief 17](../briefs/engine/done/17-engine-ui-framework.md), branch `engine-ui-framework`). The `@engine/ui` package + all six Citadel UI consumers have landed and been verified in-browser (WebGPU): resource-HUD (pilot) + all-goods strip (grain/flour/bread/wood/planks/stone/tools); building inspect-view + upgrade-button (one panel, `2cab8ae`); villager-job (`84a9ef9` — also removed the DOM `#follow-hud`); town-hall build button (+ decoupled the solo town-hall from the keep/raid anchor — civic-only; MP still anchors); build-cost-hover (build prices on hover + affordability-gated buttons + the sim `BUILD_COST` economy with a founding wood grant). **DOM-overlay removal (in progress):** **toasts MIGRATED** (2026-06-30 — in-canvas `@engine/ui` column with age-driven `opacity` fade + a hidden `#toast-live` aria-live mirror; added a reusable `opacity` channel to `renderTree`). **Build bar MIGRATED** (2026-06-30 — [build-bar.ts](../../games/citadel/client/src/ui/build-bar.ts): a bottom-left in-canvas grid of grouped TEXT buttons with its own input dispatcher + a11y mirror; tier-lock/affordability greying + active-tool highlight + cost/tier hover-info preserved; emoji icons dropped because the bitmap font is ASCII-only → see the [authored-typography-and-icons](2026-06-30-engine-ui-authored-typography-and-icons.md) todo to restore an icon grid). **Remaining DOM surfaces:** the settings modal, minimap (Canvas2D), and occupancy badges (world-anchored) still need migrating onto `@engine/ui` before "all GUI in-game" is met.
